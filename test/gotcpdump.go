...
var filter = flag.String("f", "tcp and port 8000", "BPF filter for pcap")
...
func (h *httpStreamFactory) New(net, transport gopacket.Flow) tcpassembly.Stream {
	...
	if fmt.Sprintf("%v", src) == "8000" {
		hstream.runResponse() // Important... we must guarantee that data from the reader stream is read.
	} else {
		hstream.runRequest() // Important... we must guarantee that data from the reader stream is read.
	}
